...
import { StackParamList } from '../App'; // Certifique-se de que o StackParamList está correto
...
const Home = () => {
  ...
  const [users, setUsers] = useState<any[]>([]); // Ajuste o tipo conforme necessário

  useEffect(() => {
    // Carregar usuários
    axios.get('http://192.168.0.213:3000/users')
      .then(response => {
  ...
  const handleDeleteUser = (id: number) => {
    axios.delete(`http://192.168.0.213:3000/users/${id}`)
      .then(() => {
